...
def correct_sents(input_texts, targer_texts=None, pattern_file='/home/yanghh/MiduCTC-competition/gec_error_template/templates/zh/error_templates_500.txt'):
    sentence_num, error_num = 0, 0
    pattern_set = list(load_pattern(pattern_file))
    pattern_set.sort(key=lambda x: len(x[1]) + len(x[2]), reverse= True)
    output_texts = []
        ...
        for pattern in pattern_set:
            ...
            for error_span in res:
                ...
                if not b:
                    ...
                    print(line)
                    if targer_texts:
                        print(targer_texts[i])
                    line = line.replace(error_span, correct_span)
                    print(line)
        if is_corrected:
            sentence_num += 1
        output_texts.append(line)
    return output_texts, sentence_num, error_num
