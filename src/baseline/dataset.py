...
from utils.preprocess_data import align_sequences
...
class DatasetCTC(Dataset):
    ...
    def parse_item(self, src, trg):
        ...
        # replace_idx_list, delete_idx_list, missing_idx_list = self.match_ctc_idx(src, trg)

        labels = align_sequences(src[1:], trg[1:])              # 提取编辑标签
        assert len(labels) == len(src), "标签labels长度与源句子长度不一致"

        d_tags, c_tags = [], []
        for label in labels:
            if "$KEEP" in label:
                d_tags.append(self._keep_d_tag_id)
                c_tags.append(self._keep_c_tag_id)
            elif "$DELETE" in label:
                d_tags.append(self._error_d_tag_id)
                c_tags.append(self._delete_c_tag_id)
            elif "$REPLACE" in label:
                d_tags.append(self._error_d_tag_id)
                c_tags.append(self.ctag2id.get(label, self.replace_unk_c_tag_id))
            elif "$APPEND" in label:
                d_tags.append(self._error_d_tag_id)
                c_tags.append(self.ctag2id.get(label, self.append_unk_c_tag_id))

        assert len(d_tags) == len(src), "标签labels长度与源句子长度不一致"
        assert len(c_tags) == len(src), "标签labels长度与源句子长度不一致"

        # --- 对所有 token 计算loss ---
        src_len = len(src)
        ignore_loss_seq_len = self.max_seq_len-(src_len+1)  # ex sep and pad
        # 先默认给keep，会面对有错误标签的进行修改
        d_tags = [self._loss_ignore_id] + d_tags + [self._loss_ignore_id] * ignore_loss_seq_len
        c_tags = [self._loss_ignore_id] + c_tags + [self._loss_ignore_id] * ignore_loss_seq_len
        ...
        #     c_tags[miss_idx + 1] = self.ctag2id.get(miss_char, self.append_unk_c_tag_id)
