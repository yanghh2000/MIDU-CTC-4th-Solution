...
class TrainerCtc:
    ...
    def load_texts_from_fp(file_path):
        ...
        if '.txt' in file_path:
            ...
            for line in json_data:
                ...
                # src_texts.append(line['original_text'])
                # trg_texts.append(line['correct_text'])
    ...
    def train(self):
        ...
        for epoch in range(1, self.epochs + 1):
            if self.ddp_local_rank != -1:
                self.train_ds.sampler.set_epoch(epoch)          # 使得每张卡在不同epoch获得不同数据
