...
def apply_transformation(source_token, target_token):
    ...
    # checks = [check_equal, check_verb, check_plural]
    for check in checks:
...
def align_sequences(src, trg):
    # source_sent, target_sent = args
    # check if sent is OK
    # if not is_sent_ok(src) or not is_sent_ok(trg):
    #     return None
    ...
    for idx, diff in enumerate(diffs):
        ...
        source_part = _split(" ".join(src[i1:i2]))
        target_part = _split(" ".join(trg[j1:j2]))
        if tag == 'equal':
            continue
        elif tag == 'delete':
    ...
    # get labels
    labels = convert_edits_into_labels(src, all_edits)  # 把编辑转化为具体的标签。
    labels = [label[0] for label in labels]                 # 取第一个标签
...
def convert_alignments_into_edits(alignment, shift_idx):
    ...
    if target == '$TRANSFORM_CASE_CAPITAL':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vocab_path',
                        ...
                        default="../data/output_vocabulary_chinese_char_hsk+lang8_5")  # 词表保存路径
